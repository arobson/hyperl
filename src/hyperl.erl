...
%% @doc Caclulates the route maps to feed into the middleware
%% and provides the tuple Elli needs for middleware.
-spec configure([atom()]) -> [term()].
configure(ResourceList) -> {hyperl, [{route_map, meta:get_routes(ResourceList)}]}.
...
handle(Req, Args) ->
	...
	RouteMap = proplists:get_value(route_map, Args),
	Routes = filter_routes(Method, Path, RouteMap),
